...
# categorical columns: fill with mode (most frequent value) so LabelEncoder wont crash on NaN
...
# Customer ID and Name are just unique ids they dont help predict anything so we drop them
# the assignment brief says to remove columns that do not contribute to prediction
...
# sometimes the data varies a lot like age 18 vs income 25000
# if we dont standardize the model thinks income is very important just because the number is bigger
# StandardScaler puts everything on same scale with mean=0 and std=1
...
print("\nshape of dataset")
print(df.shape)        # - shows (rows, columns). shape is a property not a function
...
lasso = Lasso(max_iter=5000) # number of iterations for convergence
